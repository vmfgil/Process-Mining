...
# --- FUNÇÃO CREATE_CARD DO CÓDIGO FUNCIONAL ---
def create_card(title, icon, chart_bytes=None, dataframe=None, body_fn=None):
    with st.container():
        # Envolvemos tudo num container para estabilidade
        if chart_bytes:
...
# --- FUNÇÕES DE ANÁLISE (COMPLETAS DO CÓDIGO ORIGINAL) ---
@st.cache_data
def run_pre_mining_analysis(dfs):
  ...
  fig, ax = plt.subplots(figsize=(8,4)); sns.boxplot(x=df_projects['actual_duration_days'], ax=ax, color="#60A5FA"); sns.stripplot(x=df_projects['actual_duration_days'], color="#1f8ef1", size=4, jitter=True, alpha=0.5, ax=ax); ax.set_title("Distribuição da Duração dos Projetos")
  ...
  fig, ax = plt.subplots(figsize=(8,4)); sns.histplot(perf_df["lead_time_days"], bins=20, kde=True, ax=ax, color="#60A5FA"); ax.set_title("Distribuição do Lead Time (dias)")
  plots['lead_time_hist'] = convert_fig_to_bytes(fig)

  fig, ax = plt.subplots(figsize=(8,4)); sns.histplot(perf_df["avg_throughput_hours"], bins=20, kde=True, color='#34D399', ax=ax); ax.set_title("Distribuição do Throughput (horas)")
  plots['throughput_hist'] = convert_fig_to_bytes(fig)

  fig, ax = plt.subplots(figsize=(8,4)); sns.boxplot(x=perf_df["avg_throughput_hours"], color='#10B981', ax=ax); ax.set_title("Boxplot do Throughput (horas)")
  plots['throughput_boxplot'] = convert_fig_to_bytes(fig)

  fig, ax = plt.subplots(figsize=(8,5)); sns.regplot(x="avg_throughput_hours", y="lead_time_days", data=perf_df, ax=ax, color='#3B82F6'); ax.set_title("Relação Lead Time vs Throughput")
  ...
  fig, ax = plt.subplots(figsize=(8,5)); sns.barplot(x=activity_counts.head(10).values, y=activity_counts.head(10).index, ax=ax, color="#3B82F6"); ax.set_title("Atividades Mais Frequentes")
...
def login_page():
    ...
    # Usamos o st.container para ter mais controlo sobre o layout central
    with st.container():
        cols = st.columns([1, 1.5, 1])
        with cols[1]:
            # Criamos uma função para o corpo do login e passamo-la para a create_card
            def login_body():
            ...
            create_card("Login", "🔑", body_fn=login_body)
...
def settings_page():
    ...
    # Aqui, não usamos o card para os uploaders para evitar problemas de layout
    ...
    if all_files_uploaded:
        def preview_body():
            if st.toggle("Visualizar as primeiras 5 linhas dos ficheiros", value=False):
                for name, df in st.session_state.dfs.items():
                    ...
                    st.markdown("---")
        create_card("Pré-visualização dos Dados", "📄", body_fn=preview_body)

        def run_analysis_body():
        ...
        create_card("Execução da Análise", "🚀", body_fn=run_analysis_body)
    else:
...
def render_pre_mining_dashboard():
    ...
    # O resto das seções do dashboard pré-mineração pode ser adicionado aqui, seguindo o mesmo padrão...
...
def render_post_mining_dashboard():
    ...
    # O resto das seções do dashboard pós-mineração pode ser adicionado aqui...
...
def main():
    if not st.session_state.authenticated:
        ...
        with st.sidebar:
            st.markdown(f"### 👤 {st.session_state.get('user_name', 'Utilizador')}")
            st.markdown("---")
            if st.button("🏠 Dashboard Geral", use_container_width=True, key="nav_main_dash"):
                st.session_state.current_page = "Dashboard"
                st.rerun()
            if st.button("⚙️ Configurações", use_container_width=True, key="nav_main_settings"):

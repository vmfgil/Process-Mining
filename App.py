...
# --- FUNÇÕES DE ANÁLISE (LÓGICA ORIGINAL E ESTÁVEL) ---
@st.cache_data
def run_analysis_and_process_data(dfs):
    # Dicionário para guardar todos os dataframes processados e artefactos estáticos
    processed_data = {}

    # --- LÓGICA DE DADOS ORIGINAL E INTOCADA ---
    # Esta secção replica a lógica do seu App.py funcional para garantir estabilidade
    ...
    # --- GERAÇÃO DE ARTEFACTOS ESTÁTICOS (PETRI NETS, GRAFOS DE REDE, ETC) ---
...
with st.sidebar:
    ...
    page = option_menu(
        menu_title=None, options=["Upload", "Resultados"], icons=["cloud-upload-fill", "bar-chart-line-fill"],
    ...
    if st.session_state.analysis_run:
        ...
        if not df_projects_filter.empty:
            project_types = ['Todos'] + sorted(df_projects_filter['project_type'].dropna().unique().tolist())
            st.session_state.selected_type = st.selectbox("Filtrar por Tipo de Projeto", project_types)
            
            min_date = df_projects_filter['start_date'].min().date()
            max_date = df_projects_filter['end_date'].max().date()
            st.session_state.date_range = st.date_input("Filtrar por Data de Conclusão", value=(min_date, max_date), min_value=min_date, max_value=max_date)

if page == "Upload":
    st.header("Upload de Ficheiros de Dados")
    st.markdown("Por favor, carregue os 5 ficheiros CSV necessários para a análise. Após o carregamento, clique no botão para iniciar o processamento.")
    ...
    if all(st.session_state.dfs[name] is not None for name in file_names):
        st.success("Todos os ficheiros foram carregados.")
        if st.button("🚀 Iniciar Análise Completa", use_container_width=True):
            with st.spinner("A processar dados e a descobrir os modelos... Isto pode demorar um momento."):
                st.session_state.processed_data = run_analysis_and_process_data(st.session_state.dfs)
                st.session_state.analysis_run = True
            st.success("✅ Análise concluída! Navegue para a página de 'Resultados'.")
            st.balloons()

elif page == "Resultados":
    st.header("Dashboard de Análise de Processos")
    ...
    if not st.session_state.analysis_run:
        ...
        if 'selected_type' in st.session_state and st.session_state.selected_type != 'Todos':
        ...
        with tab1:
            with st.expander("Análise de Performance e Casos", expanded=True):
                ...
                with c2:
                    chart = alt.Chart(df_projects_f).mark_boxplot(extent='min-max', color=PALETA_CORES[0]).encode(
            ...
            with st.expander("Análise de Atividades e Gargalos"):
                ...
                with c1:
                    ...
                    chart = alt.Chart(service_times).mark_bar(color=PALETA_CORES[1]).encode(
                ...
                with c2:
                    ...
                    chart = alt.Chart(handoff_stats).mark_bar(color=PALETA_CORES[3]).encode(
        ...
        with tab2:
            with st.expander("Descoberta de Modelos de Processo (Estático)", expanded=True):
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown("<h4><center>Modelo (Inductive Miner)</center></h4>", unsafe_allow_html=True)
                    st.image(st.session_state.processed_data['model_inductive_petrinet'], use_container_width=True)
                with c2:
                    st.markdown("<h4><center>Modelo (Heuristics Miner)</center></h4>", unsafe_allow_html=True)
            ...
            with st.expander("Análise de Performance e Recursos (Estático)"):
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown("<h4><center>Heatmap de Performance</center></h4>", unsafe_allow_html=True)
                    st.image(st.session_state.processed_data['performance_heatmap'], use_container_width=True)
                with c2:
                    st.markdown("<h4><center>Rede Social de Recursos</center></h4>", unsafe_allow_html=True)

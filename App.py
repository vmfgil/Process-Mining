...
try:
    ...
    st.error("As bibliotecas de Process Mining (pm4py) não estão instaladas.")
    st.stop()
...
def load_and_preprocess_data(uploaded_files):
    """Carrega e pré-processa todos os dados de forma robusta."""
    try:
...
def generate_pre_mining_visuals(dfs):
    ...
    fig, ax = plt.subplots(figsize=(10, 6)); sns.barplot(data=handoff_stats.head(10), y='transition', x='handoff_time_days', palette='magma', ax=ax, hue='transition', legend=False); ax.set_title('Top 10 Transições com Maior Tempo de Espera'); results['plot_08'] = fig
...
def generate_post_mining_visuals(dfs):
    results = {}
    event_log, df_tasks, df_projects, log_df, df_resources = dfs['event_log'], dfs['tasks'], dfs['projects'], dfs['log_df'], dfs['resources']
    ...
    results['metrics_inductive'] = calculate_model_metrics(event_log, net_im, im_im, fm_im)

    net_hm, im_hm, fm_hm = heuristics_miner.apply(event_log)
    results['model_02_heuristics'] = pn_visualizer.apply(net_hm, im_hm, fm_hm)
    results['metrics_heuristics'] = calculate_model_metrics(event_log, net_hm, im_hm, fm_hm)
    ...
    # --- Análise de Variantes e Conformidade ---
    ...
    fig, ax = plt.subplots(figsize=(8, 6)); ax.pie(variants_df_full['count'].head(7), labels=[f'Variante {i+1}' for i in range(7)], autopct='%1.1f%%', startangle=90); ax.set_title('Distribuição das 7 Variantes Mais Comuns'); results['chart_04_variants_pie'] = fig
    
    aligned_traces = alignments.apply(event_log, net_im, im_im, fm_im)
    fitness_values = [trace['fitness'] for trace in aligned_traces]
    fig, ax = plt.subplots(figsize=(10, 4)); sns.histplot(fitness_values, bins=20, kde=True, ax=ax, color='green'); ax.set_title('Distribuição do Fitness de Conformidade'); results['chart_05_conformance_fitness'] = fig
    ...
    fig, ax = plt.subplots(figsize=(12, 8)); sns.barplot(x='duration_hours', y='variant_str', data=variant_durations.head(10), palette='plasma', ax=ax, hue='variant_str', legend=False); ax.set_title('Duração Média das 10 Variantes Mais Lentas'); results['chart_08_variant_duration'] = fig
    ...
    fig, ax = plt.subplots(figsize=(10, 6)); sns.scatterplot(x='fitness', y='deviations', data=deviations_df, alpha=0.6, ax=ax); ax.set_title('Diagrama de Dispersão (Fitness vs. Desvios)'); results['chart_09_deviation_scatter'] = fig
    ...
    fig, ax = plt.subplots(figsize=(12, 6)); sns.lineplot(data=monthly_fitness, x='end_month', y='fitness', marker='o', ax=ax); ax.set_title('Score de Conformidade ao Longo do Tempo'); plt.xticks(rotation=45); results['chart_10_conformance_over_time'] = fig

    # --- Análise Temporal e de Linha do Tempo ---
    ...
    fig, ax1 = plt.subplots(figsize=(12, 6)); ax1.plot(kpi_temporal['completion_month'], kpi_temporal['avg_lead_time'], marker='o', color='b'); ax1.set_ylabel('Dias', color='b'); ax2 = ax1.twinx(); ax2.bar(kpi_temporal['completion_month'], kpi_temporal['throughput'], color='g', alpha=0.6); ax2.set_ylabel('Nº de Projetos', color='g'); fig.suptitle('Séries Temporais de KPIs de Performance'); results['chart_06_kpi_time_series'] = fig
    ...
    fig, ax = plt.subplots(figsize=(12, 6)); sns.lineplot(x='end_date', y='cumulative_throughput', data=df_projects_sorted, ax=ax); ax.set_title('Gráfico Acumulado de Throughput'); results['chart_11_cumulative_throughput'] = fig
    ...
    if not milestone_pairs.empty and 'duration_hours' in milestone_pairs.columns and not milestone_pairs['transition'].nunique() == 0:
        fig, ax = plt.subplots(figsize=(14, 8)); sns.boxplot(data=milestone_pairs, x='duration_hours', y='transition', ax=ax, orient='h'); ax.set_title('Análise de Tempo entre Marcos do Processo'); results['chart_12_milestone_analysis'] = fig
    else:
    ...
    fig, ax = plt.subplots(figsize=(12, 8)); sns.barplot(x=waiting_time_by_task.values, y=waiting_time_by_task.index, ax=ax, palette='viridis', hue=waiting_time_by_task.index, legend=False); ax.set_title('Tempo Médio de Espera por Atividade (dias)'); results['chart_14_avg_wait_by_activity'] = fig
    ...
    for _, trace in log_df.groupby('case:concept:name'):
        ...
        for i in range(len(resources) - 1):
            if resources[i] != resources[i+1]: handoff_counts[(resources[i], resources[i+1])] += 1
    fig, ax = plt.subplots(figsize=(14, 14)); G = nx.DiGraph();
    ...
    nx.draw_networkx_edge_labels(G, pos, edge_labels=nx.get_edge_attributes(G, 'weight'), ax=ax); results['social_network'] = fig
    ...
    fig, ax = plt.subplots(figsize=(15, 12)); nx.draw_networkx_nodes(G_bipartite, pos, nodelist=resources, node_color='skyblue', node_size=2000, ax=ax); nx.draw_networkx_nodes(G_bipartite, pos, nodelist=roles, node_color='lightgreen', node_size=4000, ax=ax); nx.draw_networkx_edges(G_bipartite, pos, width=[d['weight']*0.1 for u,v,d in G_bipartite.edges(data=True)], edge_color='gray', ax=ax); nx.draw_networkx_labels(G_bipartite, pos, font_size=9); nx.draw_networkx_edge_labels(G_bipartite, pos, edge_labels={(u,v):d['weight'] for u,v,d in G_bipartite.edges(data=True)}); ax.set_title('Rede de Recursos por Função'); results['bipartite_network'] = fig
...
if menu_selection == "1. Carregar Dados":
    ...
    if not st.session_state.analysis_complete:
        ...
        with tab1:
            sections = ["Visão Geral e KPIs", "Análise de Performance e Prazos", "Análise Organizacional, Atividades e Custos", "Análise de Gargalos e Handoffs"]
            selected_section = st.selectbox("Navegar para a secção:", sections)
            ...
            if selected_section == "Análise de Performance e Prazos":
            ...
            if selected_section == "Análise Organizacional, Atividades e Custos":
            ...
            if selected_section == "Análise de Gargalos e Handoffs":
        ...
        with tab2:
            sections = ["Descoberta de Modelos de Processo", "Análise de Variantes e Conformidade", "Análise Temporal e de Linha do Tempo", "Análise de Tempos de Espera e Recursos"]
            selected_section = st.selectbox("Navegar para a secção:", sections, key='tab2_selectbox')

            if selected_section == "Descoberta de Modelos de Processo":
            ...
            if selected_section == "Análise de Variantes e Conformidade":
            ...
            if selected_section == "Análise Temporal e de Linha do Tempo":
            ...
            if selected_section == "Análise de Tempos de Espera e Recursos":
